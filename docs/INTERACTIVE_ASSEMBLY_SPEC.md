...
19 September試作追補: 集約直後は子の表示を保ち、親の面を選んで
4枚の配置と親の記号を並べる実験を追加した。相手の模様と面内回転を選ぶと、
小さい4組の適合判定と親の適合判定を共通視点で表示する。
各判定を0/1とした積の式と、親の矢印を右向きに正規化した3種類の対応表を示す。
4枚の配置は実際の子配置の外周から導出し、面内の四隅がそろった接触について
24×24×4通りを検証する。この実験は、ずれた群接触を排除する全体の接触再帰とは
範囲を区別する。3Dの親面クリックは実験用の面選択とし、内部閲覧は「子を見る」で行う。
