...
We now give the written geometric argument for the exact curved design.
...
[registration manuscript](../strong/review/CURVED_GRID_NOTE.md) states its
hypotheses and coordinate checks separately, and remains subject to
mathematical review.
...
The constants serve these questions: $w$ sets the port's half-width,
...
At a port center $p$, choose two ordered unit axes $U,V$ in the original
...
$$X(u,v)=p+w(uU+vV)+k\delta\phi(u,v)N,$$

$$\phi(u,v)=(1-u^2)(1-v^2)(1+u/5+v/7),\qquad -1\le u,v\le1,$$

with $w=1/64$, $\delta=1/4096$, and nonzero integer key
$k\in\{-12,\ldots,-1,1,\ldots,12\}$. The port has width $2w=1/32$.
The signed displacement at its center is $k\delta$. Material lies on the
side with normal coordinate $s\le k\delta\phi(u,v)$; outside the port
boxes, retain the carrier. Thus the equation specifies which side is solid,
not just an un-oriented surface floating in space.

On the square boundary, $\phi=0$, so the cap joins the original panel.
Inside, the factors $1-u^2$ and $1-v^2$ are positive. The final factor
lies between $23/35$ and $47/35$, so it stays positive too. Positive keys
make bumps and negative keys make recesses. A uniform bound on the absolute
normal displacement is

$$h=12\delta\frac{47}{35}=\frac{141}{35840}<\frac1{64}.$$

Precisely, a port box consists of points $p+wuU+wvV+sN$ with
$|u|,|v|\le1$ and $|s|\le h$. Only inside this box do we replace the
carrier's local condition $s\le0$ by $s\le k\delta\phi(u,v)$.

The frozen records put every port square at least $19/64$ from its panel's
edges, with disjoint port boxes within each tile. Changes to material stay
...
The axes $U,V$ lie in the **base plane**, not in the tangent plane of the
curved surface. For example,
$X_u(0,0)=wU+(k\delta/5)N$ and
$X_v(0,0)=wV+(k\delta/7)N$. Their nonzero normal components encode the
asymmetry that helps distinguish the two base directions.

The notation $X_u$ means to differentiate with respect to $u$ while holding
$v$ fixed; $X_v$ does the reverse. These derivatives are the directions
of the two curves obtained by moving across the cap one coordinate at a time.

![The asymmetric cap on its bounded square and the five straight lines in its algebraic continuation.](figures/tutorial-cap-rigidity.svg)

*Figure 7. Left: the height function $\phi$ over the physical square;
the vertical scale is enlarged. Right: straight lines on the continued
polynomial graph, all in its base plane. The fifth, oblique line is outside
the physical square. It is a tool for identifying the graph, not another
feature cut into the tile.*
...
graph over the original square. Use a polynomial equation for its surface
instead. Uniformly scale **all three** coordinates by $1/w$ and put
$H=k\delta/w=k/64$. The cap lies on

$$P_H(u,v,z)=z-H\phi(u,v)=0.$$

Substituting a rigid motion into the second cap's equation gives another
degree-five polynomial $F(u,v,z)$. On the common open patch,
$F(u,v,H\phi(u,v))=0$, so the preceding polynomial fact makes this an
...
in $F$, treating $u,v$ as fixed symbols, and then set $a=H\phi(u,v)$.
...
total degree 5, as does $F$. Substituting an affine motion cannot increase
...
$5=5+\deg G$, so $G$ is a nonzero constant.

Consequently $F=0$ and $P_H=0$ describe exactly the same full surface,
including points outside the physical square. This extended surface is a
...
It has exactly five, all in the plane $z=0$:

$$u=1,\quad u=-1,\quad v=1,\quad v=-1,\quad 1+u/5+v/7=0.$$

Here is a direct check of completeness. A line has coordinates
$(u,v,z)=(x+dt,y+et,c+ft)$. Substitution into the graph equation has
degree-five coefficient proportional to $d^2e^2(d/5+e/7)$. For the height
to be linear in $t$, every coefficient of $t^2$ or higher must vanish.
The fifth-degree coefficient therefore gives three cases.

Work through $d=0$, $e\ne0$ first. Then $u=x$ is constant, so the height is

$$H(1-x^2)\bigl(1-(y+et)^2\bigr)
\bigl(1+x/5+(y+et)/7\bigr).$$

Its $t^3$ coefficient is $-H(1-x^2)e^3/7$. Since $H$ and $e$ are nonzero,
that coefficient vanishes only when $x=1$ or $x=-1$. The factor $1-x^2$
then makes the whole height zero, giving the first two straight lines.
The case $e=0$, $d\ne0$ works the same way: its cubic coefficient is
$-H(1-y^2)d^3/5$, giving $y=\pm1$ and the next two lines.

If $d,e$ are both nonzero, the fifth-degree condition gives $e=-7d/5$.
Now the last factor of $\phi$ is constant along the proposed line. The
$t^4$ coefficient is $Hd^2e^2(1+x/5+y/7)$, so this constant must vanish.
The height is again zero, on the fifth listed line. Finally, $d=e=0$
would describe a vertical line, impossible for a graph with only one height
at each $(u,v)$. We have exhausted all possible line directions.

The two pairs of parallel lines recover the square, its center, and its
base plane. A square alone allows eight symmetries: rotations by multiples
of $90^\circ$ and reflections. The remaining line singles out an ordered
choice of axes. Indeed, its equation has unequal positive coefficients
$1/5,1/7$ and constant term 1. Swapping axes or changing any signs changes
that line. Thus no nonidentity square symmetry preserves all five lines.
Only a reversal of the normal remains possible; the graph equation then
requires the corresponding reversal of the signed key.

This is why an asymmetric polynomial works where a flat face can slide.
...
a much simpler graph. Exercise 13 checks the distinction between base-plane
directions and tangent directions before you continue to Step C.
...
signed permutation matrix. To see that $t$ is integral too, use a coordinate
property of this particular design: the center $f$ of the port's unit panel
satisfies $f=p-(3U+V)/16$. Matching $p,U,V$ therefore matches $f$, and
...
For this design, every occurrence of a given signed key has the same sign,
and $\chi(-k)=-\chi(k)$. Taking determinants in the frame equations gives
...
checks of the [192 port records](../strong/audit/frozen_v1/candidate.json).
...
containing $c$. Put $\rho=1/64$. Move each coordinate of $c$, only if needed,
...
The [longer geometric scrutiny](../strong/review/GEOMETRIC_GRID_SCRUTINY.md)
and the shorter manuscript give the detailed coordinate audits. They do
not certify triangulated or printed replacements of the exact surface.
...
- **Follow the geometric proof:** [short registration manuscript](../strong/review/CURVED_GRID_NOTE.md).
...
13. **Distinguish two planes (Section 7.1).** Compute the derivatives of
    $X$ at $(0,0)$ and decide whether $U,V$ are tangent to the curved
    cap there. What happens if you drop the factor $1+u/5+v/7$ from $\phi$?
14. **Exclude an extra component (Section 7.1).** Why is
    $\sqrt3/64<1/4$ enough for the clamping argument? Why would carrier
    coverage alone, without a retained-material estimate, be insufficient?
...
13. They are $wU+(k\delta/5)N$ and $wV+(k\delta/7)N$. With nonzero
    key, neither $U$ nor $V$ alone is tangent at the center. Dropping the
    last factor makes those center slopes zero and restores the square's
    in-plane symmetries; the fifth distinguishing line also disappears.
14. Clamping changes each coordinate by at most $1/64$, keeping $y$ in
    $A$'s interior ball. The inset estimate puts the same $y$ inside $B$.
    Carrier ownership by itself does not guarantee material at a point
    removed by a recess.
...
19. $F=(z-u^2)(z+u^2)$, so $G=z+u^2$. The second factor contributes
    ...
    sets. In Step B both defining polynomials have degree 5, leaving only
