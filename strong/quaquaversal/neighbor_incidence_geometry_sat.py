"""Q019: possible-neighbor incidence plus sampled interior non-overlap.
...
This remains a necessary relaxation: optional tiles need no complete stars
yet, and the supplied interior samples detect only some overlaps. SAT is not
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'artifacts'/'neighbor_incidence_geometry_sat.json')
    ...
    paths.extend((args.baseline,args.forbidden))
    ...
    for ci,(si,row) in enumerate(chosen_rows):
        ...
        if status == z3.sat:
            ...
            active = present|{pos for pos,defs in definitions.items() if selected[defs[0][0]] in defs[0][1]}
    ...
    output = dict(scope='Finite incidence and sampled non-overlap constraints; satisfiable assignments are not tilings',
