"""Audit a whole forced-propagation layer, including zero-count categories.
...
def main():
    ...
    parser.add_argument('--input',type=Path,default=ROOT/'artifacts'/'forced_outer_layer_2.json')
    parser.add_argument('--source',type=Path,default=ROOT/'artifacts'/'forced_outer_layer.json')
    parser.add_argument('--source-poses',type=Path)
    parser.add_argument('--output',type=Path,default=ROOT/'artifacts'/'forced_outer_layer_2_audit.json')
    args = parser.parse_args()
    paths = [ROOT/'artifacts'/'closed_contact_atlas.json',ROOT/'artifacts'/'closed_star_compatibility.json',args.source,args.input]
