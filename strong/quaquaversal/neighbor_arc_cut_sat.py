"""Q023: bounded SAT refinement with complete-neighbor stars and arc cuts.
...
from neighbor_arc_oracle import ArcOracle
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'artifacts'/'neighbor_arc_cut_sat.json')
    parser.add_argument('--cuts',type=Path,default=ROOT/'artifacts'/'neighbor_star_cut_sat.json')
    parser.add_argument('--arc-cuts',type=Path,default=ROOT/'artifacts'/'neighbor_arc_cuts.json')
    ...
    initial_arc_cuts = json.loads(args.arc_cuts.read_text())
    compat_path = ROOT/'artifacts'/'closed_star_compatibility.json'
    compat = json.loads(compat_path.read_text())
    paths.extend((args.baseline,args.forbidden,args.cuts,args.arc_cuts,compat_path,ROOT/'neighbor_arc_oracle.py'))
    assert initial_cuts['sources'][args.input.name] == hashlib.sha256(args.input.read_bytes()).hexdigest()
    assert initial_arc_cuts['sources'][args.input.name] == hashlib.sha256(args.input.read_bytes()).hexdigest()
    ...
    cuts = [dict(antecedents=c['antecedents'],origin=dict(kind='audited_initial',file=path.name,index=i))
            for path,data in ((args.cuts,initial_cuts),(args.arc_cuts,initial_arc_cuts)) for i,c in enumerate(data['cuts'])]
    ...
    oracle = ArcOracle(positions,position_ids,ars,ats,d,case_maps,domain_bits,reverse)
    for ci,(si,row) in enumerate(chosen_rows):
        ...
        for step in range(args.rounds):
            ...
            arc_info = None
            if not failures:
                arc_failure,arc_info = oracle.run(requirements,intersection)
                if arc_failure is not None:
                    antecedents,certificate = arc_failure
                    assert antecedents not in cut_keys
                    cut_keys.add(antecedents)
                    failures.append(len(cuts))
                    cuts.append(dict(antecedents=antecedents,origin=dict(kind='neighbor_arc_failure',case=ci,
                                                                       round=step+1,**certificate)))
            rounds.append(dict(round=step+1,solver_status='sat',new_cuts=len(failures),cut_indices=failures,
                               installed_cuts=len(installed),neighbor_positions=len(requirements),arc=arc_info))
        ...
        result = dict(source_index=si,boundary_index=row['boundary_index'],cover_index=row['cover_index'],
                      status=('sat_passes_neighbor_arc_test' if passed else 'unknown_at_cut_round_limit') if status == z3.sat else
        ...
        print(json.dumps(dict(case=ci,status=result['status'],rounds=len(rounds),total_cuts=len(cuts))),flush=True)
    output = dict(scope='Bounded lazy complete-neighbor-star and arc cuts; UNSAT solver results await independent proof audit',
